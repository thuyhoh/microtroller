...
int GPIOx_R(unsigned short Port, unsigned short Pin){
	uint16_t status;
	if(Port == 1){// Port A
		status = (uint16_t)((GPIOA->IDR & (1<<Pin))>>Pin); 
	}
	else if(Port == 2){
		status = (uint16_t)((GPIOB->IDR & (1<<Pin))>>Pin);
	}
	else if(Port == 3){
		status = (uint16_t)((GPIOC->IDR & (1<<Pin))>>Pin);
	}else{
		status = 7;
	}
	return status;
...
void GPIOx_W(unsigned short Port, unsigned short Pin, unsigned short Status){
	if(Port == 1){
		(Status != 0) ? (GPIOA->ODR |= (unsigned int)(Status<<Pin)) : (GPIOA->ODR &= ~(unsigned int)(1<<Pin));
	}else if(Port == 2){
		(Status != 0) ? (GPIOB->ODR |= (unsigned int)(Status<<Pin)) : (GPIOB->ODR &= ~(unsigned int)(1<<Pin));
	}else if(Port == 3){
		(Status != 0) ? (GPIOC->ODR |= (unsigned int)(Status<<Pin)) : (GPIOC->ODR &= ~(unsigned int)(1<<Pin));
	}
}
...
void GPIO_ISR_init(unsigned short Port, unsigned short Pin){
	RCC->APB2ENR |= 1;
	uint16_t x = (((Port == 2))?1:0) + ((Port == 3)? 2:0);
	__disable_irq();
	AFIO->EXTICR[Pin] = x;
	EXTI->IMR 	|= (1<<Pin);
	EXTI->RTSR	|= (1<<Pin);
	if(Pin >= 0 && Pin <= 3) 					NVIC_EnableIRQ(EXTI0_IRQn);
